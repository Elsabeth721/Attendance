  ...
  try {
    ...
    setState(() {
      allRecords = (response as List).map<Map<String, dynamic>>((row) {
        final studentInfo = studentMap[row["student_id"]] ?? {"name": "Unknown", "phone": "N/A"};
        return {
...
Future<void> _exportToCSV() async {
  ...
  try {
    ...
    final fileName = 'attendance_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.csv';
...
Future<void> _exportToText() async {
  ...
  try {
    ...
    final fileName = 'attendance_report_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.txt';
    final filePath = '${directory.path}/$fileName';
...
String _generateCSVContent() {
    ...
    csv.write(filteredUniqueDates.map((d) => '"${DateFormat('MMM dd').format(d)}"').join(','));
...
String _generateTextContent() {
    ...
    text.writeln('Attendance Rate: ${((totalPresent / filteredRecords.length) * 100).toStringAsFixed(1)}%');
    text.writeln();
    ...
    students.forEach((name, dateStatuses) {
      ...
      text.writeln('     Attendance Rate: ${studentPresent + studentAbsent + studentPatient > 0 ? ((studentPresent / (studentPresent + studentAbsent + studentPatient)) * 100).toStringAsFixed(1) : 0}%');
      text.writeln();
    });
...
Future<Directory?> _getPublicDownloadsDirectory() async {
  ...
  //           const SizedBox(height: 16),
  //           Text(
  //             'Choose export format for ${filteredRecords.length} records',
  //             style: GoogleFonts.dmSans(
  //               color: AppColors.textSecondary,
  //             ),
  //           ),
...
Future<void> _updateAttendanceStatus(String attendanceId, String newStatus, String studentId, DateTime date) async {
  try {
  ...
  // void _showExportSuccess(String csvData) {
  ...
  //           Expanded(
  //             child: Text(
  //               'CSV data ready for download (${filteredRecords.length} records)',
  //               style: GoogleFonts.dmSans(),
  //             ),
  //           ),
  //         ],
                          ...
                          ...filteredUniqueDates.map((d) {
                            ...
                            return DataCell(
                              Container(
                                ...
                                decoration: BoxDecoration(
                                  color: hasRecord ? statusColor.withOpacity(0.1) : Colors.transparent,
                                  borderRadius: BorderRadius.circular(6),
                                  border: hasRecord ? Border.all(color: statusColor.withOpacity(0.3)) : null,
                                ),
                                child: Row(
                                  mainAxisSize: MainAxisSize.min,
                                  children: [
